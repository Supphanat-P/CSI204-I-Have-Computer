...
sequenceDiagram
    autonumber
    actor User as ลูกค้า (User)
    participant Web as หน้าเว็บ (Frontend)
    participant API as Backend (Node.js API)
    participant DB as Database (JSON Files)

    %% Authentication
    User->>Web: กรอกอีเมล / รหัสผ่าน
    Web->>API: POST /api/login
    API->>DB: ตรวจสอบข้อมูลผู้ใช้ (users.json)
    DB-->>API: ผลการตรวจสอบ
    API-->>Web: คืนค่า JWT Token / Login OK
    Web-->>User: แสดงหน้าจอเข้าสู่ระบบสำเร็จ

    %% Search & Browsing
    User->>Web: ค้นหา / เลือกหมวดหมู่ / ช่วงราคา
    Web->>API: GET /api/products
    API->>DB: ค้นหาและดึงข้อมูลสินค้า (products.json)
    DB-->>API: ส่งคืนรายการสินค้า
    API-->>Web: คืนค่าข้อมูลสินค้า
    Web-->>User: แสดงรายการสินค้าบนหน้าเว็บ

    %% Cart & Checkout
    User->>Web: เพิ่มสินค้าลงตะกร้า
    Note over Web: บันทึกลง LocalStorage & คำนวณราคารวม
    User->>Web: กดชำระเงิน / เลือกช่องทางชำระเงิน
    Web->>API: POST /api/orders (พร้อมข้อมูลสั่งซื้อ)
    API->>DB: ตรวจสอบสต็อก & ตัดสต็อก (products.json)
    API->>DB: บันทึกข้อมูลคำสั่งซื้อใหม่ (orders.json)
    DB-->>API: ทำรายการสำเร็จ
    API-->>Web: คืนค่าสถานะชำระเงิน & รหัสคำสั่งซื้อ
    Web-->>User: แสดงหน้าสรุปสถานะคำสั่งซื้อ & ประวัติสั่งซื้อ
```
...
sequenceDiagram
    ...
    %% Authentication
    Manager->>Web: กรอกอีเมล / รหัสผ่าน
    Web->>API: POST /api/login
    API->>DB: ตรวจสอบสิทธิ์ผู้ใช้ Manager (users.json)
    DB-->>API: ผลการตรวจสอบ
    API-->>Web: คืนค่า JWT Token (Role: Manager)

    %% Product & Stock Management
    Manager->>Web: เข้าหน้าจัดการสินค้า / สต็อก
    Web->>API: POST /api/products/create หรือ update/delete
    API->>DB: อัปเดตข้อมูลสินค้า & จำนวนคลัง (products.json)
    DB-->>API: คืนสถานะการบันทึก
    API-->>Web: คืนผลลัพธ์ Success
    Web-->>Manager: แสดงผลการจัดการสินค้าสำเร็จ

    %% Order & Shipping Management
    Manager->>Web: เข้าหน้าจัดการคำสั่งซื้อ
    Web->>API: GET /api/manager/orders
    API->>DB: อ่านรายการคำสั่งซื้อทั้งหมด (orders.json)
    DB-->>API: คืนข้อมูลคำสั่งซื้อ
    API-->>Web: แสดงรายการคำสั่งซื้อทั้งหมด
    Manager->>Web: อัปเดตสถานะจัดส่งสินค้า
    Web->>API: POST /api/manager/orders/update-status
    API->>DB: บันทึกสถานะใหม่ลง orders.json
    DB-->>API: คืนผลลัพธ์การอัปเดต
    API-->>Web: คืนสถานะการทำงาน Success
    Web-->>Manager: แจ้งเตือนอัปเดตสถานะสำเร็จ
```
...
sequenceDiagram
    ...
    %% Authentication
    Admin->>Web: กรอกอีเมล / รหัสผ่าน
    Web->>API: POST /api/login
    API->>DB: ตรวจสอบสิทธิ์ผู้ใช้ Admin (users.json)
    DB-->>API: ผลการตรวจสอบ
    API-->>Web: คืนค่า JWT Token (Role: Admin)

    %% User Account Management
    Admin->>Web: เข้าหน้าจัดการผู้ใช้งาน (Manage Users)
    Web->>API: GET /api/users
    API->>DB: ดึงรายชื่อผู้ใช้งานทั้งหมด (users.json)
    DB-->>API: คืนค่า User List
    API-->>Web: แสดงรายการบัญชีผู้ใช้
    Admin->>Web: เปลี่ยนสิทธิ์ / ระงับบัญชีผู้ใช้
    Web->>API: POST /api/users/role
    API->>DB: อัปเดตสิทธิ์ใน users.json
    DB-->>API: คืนผลลัพธ์ Success
    API-->>Web: แจ้งผลการเปลี่ยนสิทธิ์
    Web-->>Admin: แสดงผลการดำเนินรายการสำเร็จ

    %% Dashboard & Sales Summary
    Admin->>Web: เข้าหน้าดูสรุปภาพรวม (Summary Report)
    Web->>API: GET /api/products & GET /api/manager/orders
    API->>DB: อ่านข้อมูลสรุปภาพรวมยอดขาย & สต็อก
    DB-->>API: คืนข้อมูลสถิติ
    API-->>Web: แสดงผลแดชบอร์ดสรุปภาพรวม
    Web-->>Admin: แสดงรายงานสรุปสำหรับผู้บริหาร
```
